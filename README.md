...
- Optional scroll effects: parallax gradients and IntersectionObserver-powered card reveals are enabled by default via `data-scroll-effects="enabled"` on `<body>`, and can be disabled instantly by removing the attribute or setting it to `disabled` if you prefer the static experience.
