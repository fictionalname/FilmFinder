...
- Genre chips support double-click exclusion with a red state, sending TMDB `without_genres` filters while single-clicking clears any include/exclude selections.
...
- Optional “If you like this film...” strip under the provider list surfaces TMDB similar-film suggestions filtered to the active providers (remove this block by dropping any references to that heading).
