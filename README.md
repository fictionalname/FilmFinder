...
- Optional scroll effect: a layered parallax starfield (fed by the included JPG) sits behind the layout whenever `<body data-scroll-effects="enabled">` is present; remove or change that attribute for an instant return to the static gradient backdrop.
